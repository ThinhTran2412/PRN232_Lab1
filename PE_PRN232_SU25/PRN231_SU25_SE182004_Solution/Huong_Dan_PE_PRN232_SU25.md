...
Xóa hoặc comment phương thức `OnConfiguring` tự động sinh ra trong `SU25LeopardDBContext.cs`.
...
```json
{
  ...
  "Jwt": {
    "Key": "MotChuoiBiMatSieuDaiVaPhucTapChoJWTToken123456!",
    "Issuer": "PRN232",
    "Audience": "PRN232User"
  },
  "Logging": { ... }
...
## Bước 7: Viết API Controller cho CRUD (`LeopardProfileController`)

Sử dụng Data Annotation để Validate ngay trong DTO/Model, tự động trả về `HB40001`. (Cần custom thêm `InvalidModelStateResponseFactory` trong Program.cs nếu muốn chuẩn 100% format `HB40001` cho Data Annotation).
...
public class LeopardProfileController : ControllerBase
{
    ...
    {
        return Ok(_context.LeopardProfiles.ToList());
    }
    ...
    {
        var profile = _context.LeopardProfiles.Find(id);
        if (profile == null) return NotFound(new { errorCode = "HB40401", message = "Không tìm thấy tài nguyên" });
        return Ok(profile);
    }
    ...
    {
        // 1. Validation theo đề
        if (!Regex.IsMatch(model.LeopardName, @"^([A-Z0-9][a-zA-Z0-9-#]*\s*[A-Z0-9][a-zA-Z0-9-#]*)$"))
            return BadRequest(new { errorCode = "HB40001", message = "LeopardName không đúng định dạng" });
        if (model.Weight <= 15)
            return BadRequest(new { errorCode = "HB40001", message = "Weight phải lớn hơn 15" });

        _context.LeopardProfiles.Add(model);
    ...
    {
        var existing = _context.LeopardProfiles.Find(id);
        if (existing == null) return NotFound(new { errorCode = "HB40401", message = "Không tìm thấy" });
    ...
    {
        var profile = _context.LeopardProfiles.Find(id);
        if (profile == null) return NotFound(new { errorCode = "HB40401", message = "Không tìm thấy" });
...
```
*(Ghi chú: Để đúng kiến trúc 3 lớp thuần túy nhất, logic `_context.LeopardProfiles...` nên được chuyển sang các class nằm ở Project `Services`, Controller chỉ gọi Service. Tuy nhiên trong phòng thi thời gian ngắn, việc Inject thẳng DbContext hoặc viết logic đơn giản vào Controller đôi khi được chấp nhận. Khuyến nghị viết ra 1 file `ILeopardProfileService` nếu có đủ thời gian).*

---

## Bước 8: Viết script test Postman
